---
title: "Teaching"
description: "Teaching experience and mentorship."
---

<div class="award-callout">
  <span class="award-label">Award · 2024–2025</span>
  <p class="award-title">PhD TA of the Year</p>
  <p class="award-desc">Duke University Department of Statistical Science</p>
</div>

## Instructor of Record

- **STA 199**:  Intro to Data Science and Statistical Thinking, Duke University, Department of Statistical Science, Summer 2025


  Led and developed material for a course covering data wrangling, visualization, modeling, and hypothesis testing. Trained students from diverse backgrounds in R, RStudio, and Git; mentored case studies analyzing real-world data sets

  [Course Site](https://sta199-su25.github.io)

## Mentorship

- Bass Connections: Mental Health and the Justice System in Durham County.  

  Project Manager

  More information coming soon!

## Teaching Assistant at Duke

Duke University, 2023 – present

- STA 199: Intro to Data Science and Statistical Thinking (Fall 2023, Fall 2024, Fall 2025), Head TA

  Course material: Taught in R. Covers data wrangling, visualization, modeling, and hypothesis testing.

  Role: Managed coordinating of ~20 TAs accross sections. Taught multiple lab sections (~30 student each), held office hours, and graded homeworks.


## Teaching Assistant at the University of Alabama

University of Alabama, 2018 - 2021

- CS 100: Computer Science 1 for Majors

  Course material: Taught in C. A survey of programming concepts including conditionals, loops, arrays, and pointers.

  Role: Assisted at labs and graded quizzes/exams. Each semester I worked with up to 200 students across multiple sections.

- MIS 501: Application Development for the Data-Driven Organization (Master's Level)

  Course material: Introduces Python concepts for data collection and analysis.

  Role: Graded projects, homeworks, and exams. Held office hours.
